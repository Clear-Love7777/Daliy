(async function run() {
    ...
    router.post("/insertRooms", async ctx => {
        const datas = ctx.request.body.product;
        const [rs] = await connection.query("INSERT INTO rooms (roomname,roomcategory,roomdetail,price,roomID,roomstatus) VALUE ('" + datas.roomname + "','" + datas.roomcategory + "','" + datas.roomdetail + "','" + datas.price + "','" + datas.roomid + "','" + datas.roomstatus + "')");
        if (rs.affectedRows > 0) {
            ctx.body = {
                code: 0,
                data: '添加成功'
    ...
    router.post("/deleteRooms", async ctx => {
        const ID = ctx.request.body.ID
        // console.log(ID)
        let sql = "DELETE FROM rooms WHERE ??=?";
        let [rs] = await connection.query(sql, ["ID", ID]);
    ...
    router.post("/updateRooms", async ctx => {
        const data = ctx.request.body.product;
        // console.log(data)
        let sql = `UPDATE rooms SET roomstatus=${data.roomstatus} , roomID=${data.roomid} , roomname='${data.roomname}' , roomcategory='${data.roomcategory}' , price=${data.price} , roomdetail='${data.roomdetail}' WHERE ID=${data.updateid};`;
        let [rs] = await connection.query(sql);
    ...
    router.post("/deleteOrders", async ctx => {
      const ID = ctx.request.body.ID;
      // console.log(ID)
      let sql = "DELETE FROM orders WHERE ??=?";
      let [rs] = await connection.query(sql, ["ID", ID]);
